...
stlink_t *stlink_open_usb(enum ugly_loglevel verbose, bool connect_under_reset, int reset, char serial[STLINK_SERIAL_MAX_SIZE], int freq, int speed) {
    stlink_t* sl = NULL;
...
static size_t stlink_probe_usb_devs(libusb_device **devs, stlink_t **sldevs[]) {
    ...
    while ((dev = devs[i++]) != NULL) {
        ...
        stlink_t *sl = stlink_open_usb(0, 0, 1, serial, 0, 0);
