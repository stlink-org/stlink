...
static void mem_view_add_as_hex (
        GtkListStore *store,
        GtkTreeIter  *iter,
        gint          column,
        guint32       value) {
...
static gboolean stlink_gui_update_filemem_view (STlinkGUI *gui) {
    ...
    gtk_notebook_set_tab_label_text (gui->notebook,
            GTK_WIDGET (gtk_notebook_get_nth_page (gui->notebook, 1)), basename);
    g_free (basename);
...
static gpointer stlink_gui_populate_filemem_view (gpointer data) {
    ...
    if (g_str_has_suffix (gui->filename, ".hex")) {
        ...
        // so do it simple - load whole file into buffer and copy the data to the
        // destination afterwards.
        // We loose meanwhile the displaying of the progress and need double memory.
...
static void stlink_gui_set_connected (STlinkGUI *gui) {
    ...
    gtk_statusbar_push (gui->statusbar,
            gtk_statusbar_get_context_id (gui->statusbar, "conn"), "Connected");
...
static void notebook_switch_page_cb (
...
void stlink_gui_init_dnd (STlinkGUI *gui) {
    GtkTargetEntry target_list[] = { { "text/uri-list", 0, TARGET_FILENAME }, };
