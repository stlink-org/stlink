...
static int _stlink_match_speed_map(const uint32_t *map, unsigned int map_size, uint32_t khz) {
    ...
    if (speed_index == -1) {
        // This will only be here if we cannot match the slow speed. Use the slowest speed we support.
...
int _stlink_usb_set_swdclk(stlink_t* sl, uint16_t clk_divisor) {
    ...
    if (sl->version.stlink_v == 2 && sl->version.jtag_v >= 22) {
        ...
        if (speeds_size > STLINK_V3_MAX_FREQ_NB) speeds_size = STLINK_V3_MAX_FREQ_NB;
...
stlink_t *stlink_open_usb(enum ugly_loglevel verbose, int reset, char serial[STLINK_SERIAL_MAX_SIZE], int freq) {
    ...
    /* @TODO: Reading a environment variable in a usb open function is not very nice, this should be refactored
     * and moved into the CLI tools, and instead of giving USB_BUS:USB_ADDR a real stlink serial string should
     * be passed to this function. Probably people are using this but this is very odd because as programmer
     * can change to multiple busses and it is better to detect them based on serial.
     */
    ...
    if (cnt < 0) {
        ...
        if (ret != 0) {
            WLOG("Error %d (%s) opening ST-Link v%d device %03d:%03d\n", ret, strerror (errno),
                    sl->version.stlink_v, libusb_get_bus_number(list[cnt]), libusb_get_device_address(list[cnt]));
